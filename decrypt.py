"""
Solution developed by Fulvio Troiani
Email: fulviotroiani@gmail.com
Mobile: 07759138843
"""
...
def isIncremental(buffer):
    """TODO 1: Please implement a function which will:
     ...
     """
    # the elements of the buffer are appended in a list
    my_buffer = []
    for character in buffer:
        my_buffer.append(character)

    # temporary variables used for comparing the elements of the list
    first_element = 0
    second_element = 1

    # checking if the list contains incremental values
    for c in range(len(my_buffer) - 1):
        if my_buffer[second_element] - my_buffer[first_element] == 1:
...
def decryptFile(candidates):
    ...
    """
    permutation = list(
        permutations(candidates, 2))  # This function may be used to generate all permutations of candidate values.

    encrypted_file = Path('data/encrypted_file')  # path to the file to decrypt
    with open(encrypted_file, 'rb') as file:
        encrypted = file.read()  # reading the file

    for a, b in permutation:  # iterating through the elements of of permutation list
        key = a
        iv = b
        cipher = AES.new(key, AES.MODE_CBC, iv)  # creating a new cipher object
        decrypted = cipher.decrypt(encrypted)  # attempting the decryption of the file
        if isKnownHeader(decrypted):  # checking if the decryption was successful
            ext = isKnownHeader(decrypted)[1]
            # if decryption was successful appending the file extension to the header
            with open('data/decrypted'+str(ext), 'wb') as nf:
                nf.write(decrypted)
